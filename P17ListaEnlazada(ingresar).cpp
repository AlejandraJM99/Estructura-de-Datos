...
//se crean punteros del mismo tipo de la estructura nodo
	...
	public: //en la sección pública se crean los métodos a implementar.
	void agregar(int item) //Función para agregar elementos a la lista, el cual es de tipo void y recibirá un entero al que se denomina item y será dado por el usuario.
	{
...
int main()
{
	int dato,op;
	Lista list1;
	do{ //Dentro de un método do-while, se lleva a cabo un switch para poder mostrar al usuario las opciones del programa. 1.Agregar y 2.Salir del programa. Su opc se guarda en la variable op.
	...
	cin>>op;
		...
		switch(op) //la función switch funciona con la elección del usuario.
		{
			...
			cin>>dato;
			list1.agregar(dato); //Llamada al método agregar();
